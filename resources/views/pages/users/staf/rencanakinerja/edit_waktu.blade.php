...
@section('content')
    ...
    <div class="container-fluid">
        ...
        <form id="myForm" action="/staf-perencanaankerja/rencanakinerja/{{ $rencanakinerja->id }}" method="POST">
            @csrf
            @method('put')
            <div class="row mb-8">
                <div class="col-sm-12">
            ...
            <div class="row mb-8">
                <div class="col-sm-12">
                    <div class="form-group">
                        ...
                        <input type="aspek" class="form-control" id="aspek" name="aspek" value="waktu" disabled>
                    </div>
                    <div class="form-group">
                        <label for="iki">IKI</label>
                        <input type="iki" class="form-control" id="waktu_iki" name="waktu_iki" value="{{ $rencanakinerja->waktu_iki }}" required>
                    </div>
                    <div class="form-group">
                        <label for="targetmin">Target Min</label>
                        <input type="targetmin" class="form-control" id="waktu_target_min" name="waktu_target_min" value="{{ $rencanakinerja->waktu_target_min }}" required onkeypress="return (event.charCode !=8 && event.charCode ==0 || (event.charCode >= 48 && event.charCode <= 57))">
                    </div>
                    <div class="form-group">
                        <label for="targetmix">Target Max</label>
                        <input type="targetmix" class="form-control" id="waktu_target_max" name="waktu_target_max" value="{{ $rencanakinerja->waktu_target_max }}" required onkeypress="return (event.charCode !=8 && event.charCode ==0 || (event.charCode >= 48 && event.charCode <= 57))">
                    </div>
                    <div class="form-group">
                        <label for="satuan">Satuan</label>
                        <input type="satuan" class="form-control" id="waktu_satuan" name="waktu_satuan" value="{{ $rencanakinerja->waktu_satuan }}" required>
                    </div>
                </div>
            </div>
