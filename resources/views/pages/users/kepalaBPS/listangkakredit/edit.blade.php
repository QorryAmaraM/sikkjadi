...
@section('content')
    ...
    <div class = "container-fluid">
        ...
        <!-- Content Row -->
        <form id="myForm" action="/kepalabps-masterangkakredit/listangkakredit/{{ $angkakredit->id }}" method="POST">
            @csrf @method('put')
            <div class="row mb-8">
                <div class="col-sm-12">
                    <div class="form-group d-flex align-items-center">
                        <label for="searchSelect" class="col-sm-2 pl-0 col-form-label">Jenis Fungsional</label>
                        <select class="form-control flex-grow-1" data-width="75%" name="jenis_fungsional">
                    ...
                    <div class="form-group d-flex align-items-center" required>
                        ...
                        <input type="isibutir" class="form-control col-sm-10" name="isi_butir" value="{{ $angkakredit->isi_butir }}" required>
                    </div>
                    <div class="form-group d-flex align-items-center">
                        <label for="angkakredit" class="col-sm-2 pl-0 col-form-label">Angka Kredit</label>
                        <input type="text" class="form-control col-sm-10" name="angka_kredit" value="{{ $angkakredit->angka_kredit }}" onkeypress="return(event.charCode != 8 && event.charCode == 0 || (event.charCode >= 48 && event.charCode <= 57) || event.charCode == 46 || event.charCode == 44)" required="required">
                    </div>
                </div>
            </div>
