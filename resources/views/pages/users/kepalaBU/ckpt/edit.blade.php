...
@section('content')
    ...
    <div class="container-fluid">
        ...
        <!-- Content Row -->
        <form id="myForm" action="/kepalabu-ckp/ckpt/{{ $ckpt->id }}" method="POST">
            @csrf
            @method('put')
            <div class="row mb-8">
                <div class="col-sm-12">
    ...
    for (var i = 0; i < form.length; i++) {
        if (form[i].type == "text" || form[i].type == "select-one") {
            ...
            if (form[i].name !== 'keterangan' && form[i].name !== 'target_rev') {
                if (form[i].value == "") {
    ...
    if (allInputsFilled) {
        ...
        Swal.fire({
            icon: 'error',
            title: 'Oops...',
            text: 'Harap isi semua data kecuali keterangan sebelum melanjutkan.',
        });
    }
}
